...
/*   Updated: 2021/01/11 15:40:29 by abrun            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void		get_pt_a(player hero, t_point *pt_a, t_map map, int angle)
{
...
t_point			get_pt_h(t_param param, t_map map, int angle)
{
	t_point		pt_a;
	t_vect		vector;
	double		hp;
...
void			display_ray(player hero, t_point pt, t_param param, int angle)
{
	int		x;
	int		y;
	double	dist;
	...
	while (dist_1 < dist)
	{
		x -= round(2 * cos(convert_deg_in_rad(angle)));
		y -= round(2 * sin(convert_deg_in_rad(angle)));
		dist_1 = sqrt(pow(x - hero.x, 2) + pow(y - hero.y, 2));
		pos = (x * 4) + (y * param.img.size_line);
		param.img.data[pos] = 0xFF;
		param.img.data[pos + 1] = 0xFF;
		param.img.data[pos + 2] = 0xFF;
	}
	printf("pt_a : (%d, %d) et dist = %f\n", x, y, dist_1);
}

//------------------------Get_Dist-Vertical--------------------------\\

void		get_pt_a_vert(player hero, t_point *pt_a, t_map map, int angle)
{
	if (angle > 180)
	{
...
t_point		get_pt_v(t_param param, t_map map, int angle)
{
	t_point		pt_a;
	t_vect		vector;
...
void				display_pt_a(t_param param, int color, int angle, t_point pt_a)
{
	int		counter;
	int		pos;
...
void			draw_wall(t_wall wall, int pos_x, t_param *param, int color)
{
	...
	while (y < wall.height)
	{
		...
		while (x < wall.width)
		{
			pos = ((pos_x * (param->width / 20) + x) * 4) + (param->img.size_line * (start_y + y));
			param->img.data[pos] = color;
			param->img.data[pos] = color;
			param->img.data[pos] = color;
			x++;
...
void			display_multi_angle(t_param *param, int color)
{
	...
	while (counter < 10)
	{
		...
		draw_wall(param->wall, counter + 10, param, color);
		counter++;
	}
	printf("dist : %f\n", dist);
}
...
void				change_hero_pos(t_param param, int color)
{
	...
	//display_multi_angle(param, color);
}
